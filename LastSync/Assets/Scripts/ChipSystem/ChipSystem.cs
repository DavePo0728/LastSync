...
public class ChipSystem : MonoBehaviour
{
    ...
    public int _ChipSlotCount => ChipSlotCount;
    [SerializeField] private List<InventoryChipSlot> chipSlots;
    public List<InventoryChipSlot> chip_Slots => chipSlots;
    [Header("已裝備晶片清單")]
    [SerializeField] private List<ChipData> equippedChips = new List<ChipData>();
    public UnityAction<ChipData> buffUIChange;
    ...
    public void EquipChip(ChipData chip)
    {
        if (chip != null /*&& !equippedChips.Contains(chip)*/)
        {
            ...
            Debug.Log($"Equipped Chip: {chip.chipID}");
            RecalculateStats();
        }
    }
    /// <summary>
